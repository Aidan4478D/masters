...
grid of cells, as illustrated in Fig. X. Each grid cell is responsible for 
detecting objects whose center point lies within that region. A convolutional 
neural network (CNN) processes the entire image and produces a set of spatial 
feature maps from which bounding box predictions are generated at each grid 
location.

\begin{figure}[!htbp]
    \centering
    \includegraphics[height=5cm]{figures/methodology/4_1_1_grid.png}
    \caption{Conceptual grid created by YOLO architecture.}
    \label{fig:yolo-grid}
\end{figure}
